	...
	*/
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
		...
		else
		{
			...
			$Resim="dist/img/logo/omes_logo.png";	
		}
		...
		BeklemeSuresiMetni, EtiketSifirlamasifresi, BarkodlaEtiket,YAZI_RENGI,RENK,RESIM, SanalKlavye, VideoOynat) VALUES 
		...
		:YAZI_RENGI,:RENK, :RESIM, :SanalKlavye, :VideoOynat)");                     
		...
		$insertSQL->bindParam(':SanalKlavye',  $SanalKlavye);	
		if(isset($_POST['SanalKlavye']) && $_POST['SanalKlavye']=="on"){ $SanalKlavye=true;}else{ $SanalKlavye=false;}
		$insertSQL->bindParam(':VideoOynat',  $VideoOynat);	
		if(isset($_POST['VideoOynat']) && $_POST['VideoOynat']=="on"){ $VideoOynat=true;}else{ $VideoOynat=false;}
	...
	if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
		...
		if((!empty($_FILES["RESIM"]) and $_FILES["RESIM"]["tmp_name"]!="") and ($_POST)){
			...
			BeklemeSuresiMetni=:BeklemeSuresiMetni, EtiketSifirlamasifresi=:EtiketSifirlamasifresi, BarkodlaEtiket=:BarkodlaEtiket, SanalKlavye=:SanalKlavye, VideoOynat=:VideoOynat WHERE KID=:KID");
			...
			$updateSQL->bindParam(':SanalKlavye',  $SanalKlavye);	
			if($_POST['SanalKlavye']=="on"){ $SanalKlavye=true;}else{ $SanalKlavye=false;}
			$updateSQL->bindParam(':VideoOynat',  $VideoOynat);	
			if($_POST['VideoOynat']=="on"){ $VideoOynat=true;}else{ $VideoOynat=false;}
...
<?php if(isset($_GET["KioskEkle"]) and $_GET["KioskEkle"]=="gnc" )
	...
	<div id="gnc" class="alert alert-success"><strong><?php echo $diller['guncellemeBasariliMesaj']; ?></strong></div>
	<?php
	}
?>
<!-- Jquery ile fadein fadeout için -->
<?php if(isset($_GET["KioskEkle"]) and $_GET["KioskEkle"]=="SilEksik" )
	...
	<div id="hata" class="alert alert-danger"><strong><?php echo $diller['gecerliBirKioskSec']; ?></strong></div>
	<?php
	}
	...
	<div id="eklendi" class="btn btn-danger"><?php echo "#".$_GET['?KioskSil']; ?> <?php echo $diller['silindiMesaj']; ?><strong> <?php echo $diller['kioskSilmeNotu']; ?></strong></div>
	<?php
	}
?>
<div class="panel panel-green">
	<div class="panel panel-heading"><?php echo $diller['kioskEkrani']; ?></div>
		...
		<div class="table-responsive">
			<form name="form" id="form">  
				<select class="form-control btn btn-success" role="menu" name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenuGo('jumpMenu','parent',0)">
					<option value="?KioskEkle"><?php echo $diller['kioskBiletMakinesiSec']; ?></option>
					<?php
			...
			<a href="?KioskSil=<?php if(isset($_GET['Kiosk'])){echo $_GET['Kiosk'];} ?>" class="form-control btn btn-danger btn-md" id="sprytrigger1" onClick="return confirm('<?php echo $diller['silmePopupMesaj']; ?>');" role="button"><?php echo $diller['kioskBiletMakinesiSil']; ?></a> 
		<?php echo $diller['kioskBiletMakinesiMesaj']; ?></div>
	</div>
</div>
	...
	<form method="post" name="form1" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data" >
		<div class="form-group">
			<div class="panel panel-grey">
				<div class="panel panel-heading">1-<?php echo $diller["kioskAyarlari"]; ?></div>
				<div class="table-responsive">
					<table class="table table-hover">
						<tr>
							<th>Kiosk ID:</th>
							<td colspan="3"><?php if(isset($_GET['Kiosk'])){ echo "<span class='alert alert-danger'><strong># ".$_GET['Kiosk']." (".$diller['kioskId'].")</strong></span>"; }else { echo "<span class='btn btn-danger'><strong>Henüz Bir Kiosk Seçmediğiniz İçin İşlem Yapamazsınız.</strong></span>"; } ?></td>
						</tr>
						<tr>
							<th><?php echo $diller["kioskUstBaslik"];?>:</th>
							...
							<span id="countsprytextarea1">&nbsp;</span><span class="textareaRequiredMsg"><?php echo $diller["bosgecilemez"];?></span><span class="textareaMaxCharsMsg"><?php echo $diller["maxKarakter"];?></span></span></td>
							<th><?php echo $diller["kioskAltBaslik"];?>:</th>
						...
						<tr >
							<th valign="top"><?php echo $diller["ogleArasiMesaji"];?>:</th>
							<td><span id="sprytextarea3">
								<textarea name="MESAJ_OGLE" class="form-control" cols="30" rows="3"><?php if($row_KioskAyar['MESAJ_OGLE']!=""){ echo $row_KioskAyar['MESAJ_OGLE'];}else { echo $diller["ogleArasi"]; } ?></textarea>
							<span id="countsprytextarea3">&nbsp;</span><span class="textareaMaxCharsMsg"><?php echo $diller["maxKarakter"];?></span></span></td>
							<th valign="top"><?php echo $diller["sistemKapaliMesaji"];?>:</th>
							<td><span id="sprytextarea4">
								<textarea name="MESAJ_SISTEM_KAPALI" class="form-control" cols="30" rows="3"><?php if($row_KioskAyar['MESAJ_SISTEM_KAPALI']!=""){echo $row_KioskAyar['MESAJ_SISTEM_KAPALI'];} else { echo $diller["sistemKapali"]; } ?></textarea>
							<span id="countsprytextarea4">&nbsp;</span><span class="textareaMaxCharsMsg"><?php echo $diller["maxKarakter"];?></span></span></td>
						</tr>
						<tr>
							<th valign="top"><?php echo $diller["servisKapaliMesaji"];?>:</th>
							<td><span id="sprytextarea5">
								<textarea name="MESAJ_SERVIS_KAPALI" class="form-control" cols="30" rows="3"><?php if($row_KioskAyar['MESAJ_SERVIS_KAPALI']!=""){ echo $row_KioskAyar['MESAJ_SERVIS_KAPALI'];} else { echo $diller["servisKapali"]; } ?></textarea>
							<span id="countsprytextarea5">&nbsp;</span><span class="textareaMaxCharsMsg"><?php echo $diller["maxKarakter"];?></span></span>
							</td>
							<td colspan="2">       
								<table class="table">
									<tr>
										<th colspan="2"><?php echo $diller["biletBasmaAraligi"];?>(sn):
										</th>
										<td colspan="2"> <input class="form-control" type="number" min="1" max="100" name="GECIKME" value="<?php if($row_KioskAyar['GECIKME']!=""){echo $row_KioskAyar['GECIKME'];}else { echo 1; } ?>" size="32"></td>
									</tr>
									<tr>
									...
									<tr>
										<th><?php echo $diller["barkodlaEtiket"];?>:</th>
										...
										<th><?php echo $diller["sanalKlavye"];?>:</th>
										<td><label class="switch">
										<input class="form-control" type="checkbox" name="SanalKlavye" <?php if ($row_KioskAyar['SanalKlavye']=='1') {echo "checked=\"checked\"";} ?>><span class="slider round"></span></label></td>
									</tr>
									<tr>
										<th><?php echo $diller["videoOynat"];?>:</th>
										<td><label class="switch">
										<input class="form-control" type="checkbox" name="VideoOynat" <?php if ($row_KioskAyar['VideoOynat']=='1') {echo "checked=\"checked\"";} ?>><span class="slider round"></span></label></td>
										<th></th>
										<td></td>
									</tr>
								</table></td>
		...
		<div class="form-group">
			<div class="panel panel-blue">
				<div class="panel panel-heading">2-Kiosk <?php echo $diller["tasarimBilgileri"];?></div>
				<div class="panel body table-responsive">
					<table class="table table-hover">
						<tr valign="baseline">
							<th> <?php echo $diller["kayanYaziTipi"]; ?> (Font):</th>
						...
						<tr valign="baseline">
							<th><?php echo $diller["yaziBoyutu"]; ?>:</th>
						...
						<tr valign="baseline">
							...
							ms </td>
							<th ><?php echo $diller["altBaslikKaymaHizi"]; ?>:</th>
						...
						<tr valign="baseline">
							<th><?php echo $diller["ustBaslikKaymaYonu"]; ?>:</th>
							<td>
								<table>
									<tr>
										<td><?php echo $diller["sola"]; ?></td>
										<td><?php echo $diller["saga"]; ?></td>
									</tr>
									<tr>
										<td><label class="switch">
							...
							<th><?php echo $diller["altBaslikKaymaYonu"]; ?>:</th>
							<td>
								<table>
									<tr>
										<td><?php echo $diller["sola"]; ?></td>
		...
		<div class="panel body table-responsive" style="overflow:auto;">
			<table class="table table-hover">
				<tr>
					<th valign="baseline">Kiosk <?php echo $diller["arkaplanResmi"]; ?>:</th>
					...
					<th><?php echo $diller['resimHizalamaYonu']; ?>:</th>
					<td>
		...
		<div class="form-group">
			<div class="panel panel-blue">
				<div class="panel panel-heading">3-<?php echo $diller["etiketTasarimBilgisi"]; ?></div>
				<div class="panel body table-responsive">
					<table class="table table-hover">
						<tr valign="baseline">
							<th><?php echo $diller["yukseklik"]; ?>:</th>
							<th><?php echo $diller["onizlemeSuresi"]; ?>:</th>
							<th><?php echo $diller["ruloEtiketAdeti"]; ?>:</th>
						...
						<tr valign="baseline">
							...
							<td><input type="number" class="form-control" min="1" max="9999" name="TagPreviewTimerInterval" value="<?php if($row_KioskAyar['TagPreviewTimerInterval']!=""){echo $row_KioskAyar['TagPreviewTimerInterval']; }else { echo 1; }?>" >sn </td>
							<th><?php echo $diller["maxRuloAdeti"]; ?>:</th>
						...
						<tr valign="baseline">
							<th><?php echo $diller["genislik"]; ?>:</th>
							<th><?php echo $diller["onizlemeOrani"]; ?>:</th>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr valign="baseline">
		...
		<div class="form-group">
			<div class="panel panel-red">
				<div class="panel panel-heading">4-<?php echo $diller["digerBilgiler"]; ?></div>
				<div class="panel body table-responsive">
					<table class="table table-hover">
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller["sorumluPersonel"]; ?></th>
						...
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller["bitikEtiketMesaji"]; ?>:</th>
							<td><input class="form-control" type="text" maxlength="100" name="TagOverFlowMessage" value="<?php if($row_KioskAyar['TagOverFlowMessage']!=""){echo $row_KioskAyar['TagOverFlowMessage'];}else {echo $diller["kagitBittiMesaji"];} ?>" size="32">
							</td>
							<th><?php echo $diller["beklemeSuresiMesaji"]; ?>:</th>
							<td><input class="form-control" type="text" maxlength="100" name="BeklemeSuresiMetni" value="<?php if($row_KioskAyar['BeklemeSuresiMetni']!=""){echo $row_KioskAyar['BeklemeSuresiMetni']; } else {echo $diller["beklemeSuresiMesaji"]; } ?>" size="32"></td>
						</tr>
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller['altButonBeklemeSuresi']; ?>(sn):</th>
							...
							<th><?php echo $diller["etiketSifirlamaSifresi"]; ?>:</th>
							...
							<span class="textfieldRequiredMsg"><?php echo $diller["birdegergiriniz"]; ?></span><span class="textfieldInvalidFormatMsg"><?php echo $diller["gecersizformat"]; ?></span></span></td>
						</tr>
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller['solButonAdeti']; ?>:</th>
							<td><input class="form-control" type="number" min="0" max="100"  name="SOL_BTN_ADET" value="<?php if($row_KioskAyar['SOL_BTN_ADET']!=""){echo $row_KioskAyar['SOL_BTN_ADET'];}else {echo 0;} ?>" size="32">
							yok </td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller['sagButonAdeti']; ?>:</th>
							<td><input class="form-control" type="number" min="0" max="100" name="SAG_BTN_ADET" value="<?php if($row_KioskAyar['SAG_BTN_ADET']!=""){ echo $row_KioskAyar['SAG_BTN_ADET'];} else {echo 0; } ?>" size="32">
							yok </td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller['solBosluk']; ?>:</th>
							<td><input class="form-control" type="number" min="0" max="100" name="SOL_PADDING" value="<?php if($row_KioskAyar['SOL_PADDING']!=""){ echo $row_KioskAyar['SOL_PADDING']; } else{ echo 0; }?>" size="32">
							yok </td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr valign="baseline">
							<th valign="baseline"><?php echo $diller['sagBosluk']; ?>:</th>
							<td><input class="form-control" type="number" min="0" max="100" name="SAG_PADDING" value="<?php if($row_KioskAyar['SAG_PADDING']!="") {echo $row_KioskAyar['SAG_PADDING'];} else { echo 0;} ?>" size="32">
							yok </td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						...
						<tr valign="baseline">
							<td colspan="4" valign="baseline"><?php if(isset($_GET['KioskEkle']) and  empty($row_KioskAyar['KID']) and isset($_GET['Kiosk'])){?>
								<input class="form-control btn btn-success" type="submit" value="<?php echo $diller['ekle']; ?>">
								<input type="hidden" name="KID" value="<?php echo $_GET['Kiosk']; ?>">
								<input type="hidden" name="MM_insert" value="form1">
								<?php } else if(isset($_GET['Kiosk']) and  !empty($row_KioskAyar['KID'])){?>    
								<input type="submit" class="form-control btn btn-primary" value="<?php echo $diller['guncelle']; ?>">
